...
from autogluon.task.tabular_prediction.hyperparameter_configs import hyperparameter_config_dict
...
from frameworks.shared.callee import result, output_subdir, utils
...
class AutogluonRunner:
    ...
    def run(self, dataset, config):
        ...
        model_type = training_params.pop('benchmark_model_type', None)

        if model_type is None:
            hyperparameters = None
        else:
            frameworks_mappings = {
                **{k: {k: v} for k, v in hyperparameter_config_dict['default'].copy().items()},
                **{
                    'fastai': {NNFastAiTabularModel: {}},
                }}
            hyperparameters = frameworks_mappings[model_type]
        ...
        self.save_artifacts(predictor, leaderboard, config)
    ...
    def save_artifacts(self, predictor, leaderboard, config):
